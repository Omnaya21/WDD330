...
export default class Hikes {
  ...
  // outside of the module. This also sets me up nicely if my data were to move. I can just change this method to the new source and everything will still work if 
  // I only access the data through this getter.
  getAllHikes() {
    return hikeList;
  }
  ...
  showComments() {
    const storedComments =  this.lsHelper.load('hikes');

    if(storedComments.length) {
      const ulParent = document.getElementById('commentsList');
  ...
  buildBackButton() {
    ...
    backButton.addEventListener('click', () => {
      this.showHikeList();
      document.getElementById('newCommentForm').classList.add('hide');
      this.showComments();
    })
    return backButton;
  }
  ...
  addComment() {
    ...
    const hikeName = document.getElementsByTagName('h2')[0].textContent; 
    const comment = new Comment('hikes', hikeName, newCommentInput.value);
    const commentsList =  lsHelper.load('hikes');

    commentsList.push(comment);
    lsHelper.save('hikes', commentsList);
...
function showCommmentSection() {
  document.getElementById('newCommentForm').classList.remove('hide');
}
